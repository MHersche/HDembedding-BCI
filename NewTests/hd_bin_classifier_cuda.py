...
from nn_unsupervised import proj_trainer_unsupervised
...
class hd_classifier(lsh):
	...
	def fit_learn_unsup(self,samples,labels,n_iter=0):
		'''	Training of HD classifier with unsupervised learning
		Parameters
		----------
		samples: feature sample, numpy array shape 	spatial:(NO_samples,temp_dim,spat_dim,N_feat) 
													single:(NO_samples,temp_dim,N_feat)
		...
		unsup_model = proj_trainer_unsupervised(N_feat,N_bands,self.HD_dim,self.NO_classes,N_epoch,self.device,n_inst=NO_samples)
		unsup_model.train(samples_flat,labels_flat)

		self.ClassItem, self.proj_mat,self.enc_vec = unsup_model.get_params()
	...
	def spat_bind_transform(self,sample, clipping=True): 
		...
		# transform every spatial dimension and add together 
		for item in range(self.spat_dim): 
			hd_vec= self.xor(hd_vec,self.xor(self.enc_vec[:,item],self.encode(sample[item])))
